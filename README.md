...
`tests/failure.test.ts` proves failures can't strand a delivery: a transport-level failure (connection refused, timeout — no HTTP response at all) requeues the delivery with backoff and eventually marks it `dead` at `max_attempts`, and a malformed delivery record (missing url or secret) is marked `dead` immediately instead of being retried forever.
...
**How are replay attacks prevented?** The signature alone doesn't stop an attacker from capturing a valid webhook and re-sending it. The fix is a three-part verification in the receiver. First, the signature is checked. The string that's signed isn't just the payload, but a combination of the payload, a timestamp, and a random nonce, all generated by the dispatcher for each attempt. This means the signature covers the timestamp and nonce, so they can't be tampered with. Second, the receiver checks if the timestamp is fresh (e.g., within 5 minutes), which bounds the attack window. Third, it checks if the nonce has been seen before by using a Redis `SET` with the `NX` (not exists) option and an expiry equal to the tolerance window. This final check closes the replay vulnerability within the window.
